...
# 64 "../../VLCBlib_PIC/can18_can_2.c" 2
...
# 124 "../../VLCBlib_PIC/can.h"
typedef enum CanidResult {
    CANID_FAIL,
    CANID_OK
} CanidResult;
# 161 "../../VLCBlib_PIC/can.h"
enum CAN_OP_MODES
{
...
# 1 "../../VLCBlib_PIC/messageQueue.h" 1
# 54 "../../VLCBlib_PIC/messageQueue.h"
...
# 73 "../../VLCBlib_PIC/can18_can_2.c" 2
...
static void canPowerUp(void) {
    ...
    temp = readNVM(EEPROM_NVM_TYPE, 0x3FE);
    if (temp < 0) {
...
static DiagnosticVal * canGetDiagnostic(uint8_t index) {
    ...
    switch (index-1) {
        case 0x02:
            canDiagnostics[0x02].asUint = C1TRECU;
            break;
        case 0x03:
            if (! C1FIFOSTA2Lbits.TFNRFNIF) {
                canDiagnostics[0x03].asInt = 32;
            } else {
                i16 = (int16_t)((C1FIFOUA2 - ((0x3BB0 +((8 +8)*4))+((8 +8)*1)))/(8+8));
                i16 = (int16_t)(i16 - C1FIFOSTA2Hbits.FIFOCI);
                if (i16 < 0) i16 += 32;
                canDiagnostics[0x03].asInt = i16;
            }
            break;
        case 0x06:
            if (C1FIFOSTA3Lbits.TFERFFIF) {
                canDiagnostics[0x06].asInt = 32;
            } else {



                i16 = (int16_t)(((((0x3BB0 +((8 +8)*4))+((8 +8)*1))+((8 +8)*32)) - C1FIFOUA3)/(8+8));
                i16 += C1FIFOSTA3Hbits.FIFOCI;
                if (i16 < 0) i16 += 32;
                canDiagnostics[0x06].asInt = i16;
            }
            break;
        case 0x01:
            canDiagnostics[0x01].asUint = C1BDIAG0Hbits.NTERRCNT;
            break;
        case 0x00:
            canDiagnostics[0x00].asUint = C1BDIAG0Lbits.NRERRCNT;
            break;
...
static SendResult canSendMessage(Message * mp) {
    ...
    txFifoObj[12] = mp->bytes[3];
    txFifoObj[13] = mp->bytes[4];
    txFifoObj[14] = mp->bytes[5];
    txFifoObj[15] = mp->bytes[6];
...
static void handleSelfEnumeration(uint8_t receivedCanId) {
    ...
    switch (enumerationState) {
        ...
        case ENUMERATION_IN_PROGRESS_TX_WAITING:
            ( enumerationResults[receivedCanId>>3] |= ( 1<<(receivedCanId & 0x07) ) );
            break;
        case NO_ENUMERATION:
