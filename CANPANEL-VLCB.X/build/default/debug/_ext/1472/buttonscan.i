...
ByteBits keyInputState[8];
ByteBits keyOutputState[8];
ByteNibbles debounceCounters[8*8/2];
...
void initKeyscan(void) {
    ...
    for ( col = 0; col < 8; col++) {
        strobeMask = ~((0b00000001 << col) & 0b11111111);
        LATC |= 0b11111111;
        LATC &= strobeMask;


        keyInputState[col].val = 0;
        keyInputState[col].bits.b0 = PORTBbits.RB0;
        keyInputState[col].bits.b1 = PORTBbits.RB1;
        keyInputState[col].bits.b2 = PORTBbits.RB4;
        keyInputState[col].bits.b3 = PORTBbits.RB5;
        keyInputState[col].bits.b4 = PORTAbits.RA5;
        keyInputState[col].bits.b5 = PORTAbits.RA3;
        keyInputState[col].bits.b6 = PORTAbits.RA1;
        keyInputState[col].bits.b7 = PORTAbits.RA0;
...
void keyScan( void ) {
    ...
    for ( col = 0; col < 8; col++) {
        ...
        if (keyInputState[col].bits.b0 != PORTBbits.RB0) {
            if (debounceCounters[(((col)*8 + (0))/2)].nibbles.lower == 4) {
                debounceCounters[(((col)*8 + (0))/2)].nibbles.lower = 0;
                keyInputState[col].bits.b0 = PORTBbits.RB0;

                flags = (uint8_t)getNV(8 +((col)*8 + (0)));
                if (flags & 0x08) {
                    keyOutputState[col].bits.b0 = ! keyOutputState[col].bits.b0;
                } else {
                    if (flags & 0x04) {
                        keyOutputState[col].bits.b0 = PORTBbits.RB0;
                    } else {
                        keyOutputState[col].bits.b0 = ! PORTBbits.RB0;
                    }
                }
                sendPBEvent(((col)*8 + (0)), keyOutputState[col].bits.b0, flags);
            } else {
                debounceCounters[(((col)*8 + (0))/2)].nibbles.lower++;
            }
        }
        if (keyInputState[col].bits.b1 != PORTBbits.RB1) {

        }
        if (keyInputState[col].bits.b2 != PORTBbits.RB4) {

        }
        if (keyInputState[col].bits.b3 != PORTBbits.RB5) {

        }
        if (keyInputState[col].bits.b4 != PORTAbits.RA5) {

        }
        if (keyInputState[col].bits.b5 != PORTAbits.RA3) {

        }
        if (keyInputState[col].bits.b6 != PORTAbits.RA1) {

        }
        if (keyInputState[col].bits.b7 != PORTAbits.RA0) {
...
void sendPBEvent(uint8_t pb, uint8_t state, uint8_t flags) {
    if (state) {
        if (flags & 0x01) {
            sendProducedEvent((Happening)((pb+1)), EVENT_ON);
        }
    } else {
        if (flags & 0x02) {
            sendProducedEvent((Happening) ((pb+1)), EVENT_OFF);
        }
    }
}
