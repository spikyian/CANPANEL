...
typedef uint8_t DigitMap[8];
typedef DigitMap LedsMap[2];
...
void runLedTest( uint8_t testPasses ) {
    ...
    for (passCount = 0; passCount < testPasses; passCount++) {
        for (digCount = 0; digCount < 8; digCount++) {
            ...
            while (segCount != 0) {
                sendMxCmd( 0X60 + digCount, segCount);
                doSwDelay( 500 );
                segCount = (uint8_t)(segCount == 0x80 ? 1 : segCount<<1);
                if (segCount == 0x80)
                    segCount = 0;
            }
            sendMxCmd( 0X60 + digCount, 0 );
        }
    }
}
...
Word ledTestCycle(Word testStatus ) {
    ...
    segCount = (uint8_t)(segCount == 0 ? 1 : segCount<<1);
...
void showTestX(void) {
    sendMxCmd( 0X60, 0xC0);
    sendMxCmd( 0X60 + 1, 0x21);
    sendMxCmd( 0X60 + 2, 0x12);
    sendMxCmd( 0X60 + 3, 0x0C);
    sendMxCmd( 0X60 + 4, 0x0c);
    sendMxCmd( 0X60 + 5, 0x12);
    sendMxCmd( 0X60 + 6, 0x21);
    sendMxCmd( 0X60 + 7, 0xC0);
}
...
void clearAllLeds(void) {
    ...
    for (digCount = 0; digCount < 8; digCount++) {
        regadr = 0X60 + digCount;
        sendMxCmd( regadr, 0);
     }
     memset( (void *) ledsMap, 0, sizeof(ledsMap) );
}
...
void setOn(uint8_t ledNumber) {
    ...
    sendMxCmd( 0X20 + digNum, ledsMap[0][digNum]);
    sendMxCmd( 0x40 + digNum, ledsMap[1][digNum]);
}
...
void displayNumber( uint16_t toDisplay, uint8_t offset, uint8_t digits, uint8_t format ) {
    uint8_t byteToDisplay;

    byteToDisplay = toDisplay >> 8;
...
void displayDigit( uint8_t toDisplay, uint8_t offset ) {
    ...
    sendMxCmd( 1, decodeMode);
    sendMxCmd( 0X60 + offset, toDisplay);
}
